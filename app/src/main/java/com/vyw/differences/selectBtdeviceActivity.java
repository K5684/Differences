    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (pairedDevices.size() > 0) {
            ...
            for (BluetoothDevice device : pairedDevices) {
                ...
                BtDeviceInfoModel deviceInfoModel = new BtDeviceInfoModel(deviceName,deviceHardwareAddress);
                deviceList.add(deviceInfoModel);
            }
            // Display paired device using recyclerView
            RecyclerView recyclerView = findViewById(R.id.recyclerViewDevice);
            recyclerView.setLayoutManager(new LinearLayoutManager(this));
            BtDeviceListAdapter deviceListAdapter = new BtDeviceListAdapter(this,deviceList);
